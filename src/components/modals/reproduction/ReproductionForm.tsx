...
export function ReproductionForm({
  ...
  const calculateCalvingDates = (
    ...
    const calculateDServices = (
      ...
      // Offsets based on protocol
      // Sync D10: d8(+8), d10(+10), d22(+32), d30(+40), d32(+42), nm35(+45), nm80(+90)
      // Sync D11: d9(+9), d11(+11), d23(+34), d32(+42), d34(+44), nm37(+47), nm82(+92)

      return {
        d8_date: addDays(d0Date, isD11 ? 9 : 8),
        d10_date: addDays(d0Date, isD11 ? 11 : 10),
        d22_date: addDays(d0Date, isD11 ? 34 : 32),
        d30_date: addDays(d0Date, isD11 ? 42 : 40),
        d32_date: addDays(d0Date, isD11 ? 44 : 42),
        natural_mating_d35_entry: addDays(d0Date, isD11 ? 47 : 45),
        natural_mating_d80_exit: addDays(d0Date, isD11 ? 92 : 90),
        d110_date: addDays(d0Date, 120), // Keeps same offset? Usually diagnosis is around 120d? User didn't specify change for D110.
      };
    };
    ...
    const handleChange = (field: keyof ReproductionEvent, value: any) => {
      setFormData((prev) => {
        ...
        if ((field === "d0_date" && value) || field === "protocol_name") {
          ...
          const protocol =
            field === "protocol_name" ? value : prev.protocol_name;
    ...
    const cleanData = (data: Partial<ReproductionEvent>) => {
      ...
      (Object.keys(cleaned) as Array<keyof ReproductionEvent>).forEach(
        (key) => {
          if (cleaned[key] === "") {
            // @ts-ignore
