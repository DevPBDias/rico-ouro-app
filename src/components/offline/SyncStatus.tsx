...
export function SyncStatus() {
  const { isOnline, isSyncing, lastSync, syncNow, getPendingChanges } =
    useOfflineStatus();
  const [pendingChanges, setPendingChanges] = useState<{
    ...
    vaccines: number;
    farms: number;
  }>({ total: 0, animals: 0, vaccines: 0, farms: 0 });
            ...
            <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
              ...
              <div className="grid grid-cols-2 gap-1 text-xs text-gray-500 mt-1">
                {pendingChanges.animals > 0 && (
                  <span>
                    • {pendingChanges.animals} animal
                    {pendingChanges.animals !== 1 ? "s" : ""}
                  </span>
                )}
                {pendingChanges.vaccines > 0 && (
                  <span>
                    • {pendingChanges.vaccines} vacina
                    {pendingChanges.vaccines !== 1 ? "s" : ""}
                  </span>
                )}
                {pendingChanges.farms > 0 && (
                  <span>
                    • {pendingChanges.farms} fazenda
                    {pendingChanges.farms !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
            </div>
