...
export async function replicateAnimalMetricCE(
  db: MyDatabase,
  supabaseUrl: string,
  supabaseKey: string
) {
  const collection = db.animal_metrics_ce;
  const replicationIdentifier = "animal-metrics-ce-replication-v1";
  ...
  const replication = replicateRxCollection({
    ...
    push: {
      async handler(rows) {
        ...
        const response = await fetch(
          `${supabaseUrl}/rest/v1/animal_metrics_ce`,
          {
            method: "POST",
            headers: {
              ...headers,
