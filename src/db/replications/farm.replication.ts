...
export async function replicateFarms(
  db: MyDatabase,
  supabaseUrl: string,
  supabaseKey: string
) {
  const collection = db.farms;
  const replicationIdentifier = "farms-replication-v1";
  ...
  const replication = replicateRxCollection({
    ...
    pull: {
      async handler(checkpoint, batchSize) {
        ...
        const response = await fetch(
          `${supabaseUrl}/rest/v1/farms?select=*&order=updated_at.asc&limit=${batchSize}&updated_at=gt.${encodedDate}`,
          { headers }
        );
    ...
    push: {
      async handler(rows) {
        ...
        const response = await fetch(`${supabaseUrl}/rest/v1/farms`, {
          method: "POST",
          headers: {
            ...headers,
