...
export async function replicateReproductionEvents(
  db: MyDatabase,
  supabaseUrl: string,
  supabaseKey: string
) {
  const collection = db.reproduction_events;
  const replicationIdentifier = "reproduction-events-replication-v1";
  ...
  const replication = replicateRxCollection({
    ...
    pull: {
      async handler(checkpoint, batchSize) {
        ...
        const response = await fetch(
          `${supabaseUrl}/rest/v1/reproduction_events?select=*&order=updated_at.asc&limit=${batchSize}&updated_at=gt.${encodedDate}`,
          { headers }
        );
    ...
    push: {
      async handler(rows) {
        ...
        const response = await fetch(
          `${supabaseUrl}/rest/v1/reproduction_events`,
          {
            method: "POST",
            headers: {
              ...headers,
