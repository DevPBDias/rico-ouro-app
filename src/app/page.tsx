...
export default function OpeningPage() {
  ...
  const totalRemote = totalRemoteAnimals + totalRemoteVaccines + totalRemoteFarms + totalRemoteMatriz;
        ...
        <div className="absolute bottom-10 left-0 right-0 flex flex-col items-center justify-center text-white z-10 px-8">
          <div className="bg-black/40 backdrop-blur-md p-6 rounded-2xl w-full max-w-md border border-white/10 shadow-2xl">
            ...
            <div className="flex justify-between text-xs text-white/60">
              ...
              <span>Total: {isLoadingRemote ? "..." : totalRemoteAnimals}</span>
            </div>
            <div className="flex justify-between text-xs text-white/60">
              <span>Vacinas</span>
              <span>{totalVaccines} itens baixados</span>
              <span>Total: {isLoadingRemote ? "..." : totalRemoteVaccines}</span>
            </div>
            <div className="flex justify-between text-xs text-white/60">
              <span>Fazendas</span>
              <span>{totalFarms} itens baixados</span>
              <span>Total: {isLoadingRemote ? "..." : totalRemoteFarms}</span>
            </div>
            <div className="flex justify-between text-xs text-white/60">
              <span>Matriz</span>
              <span>{totalMatriz} itens baixados</span>
              <span>Total: {isLoadingRemote ? "..." : totalRemoteMatriz}</span>
            </div>
            <div className="flex justify-between text-xs text-white/60">
              <span>Total</span>
