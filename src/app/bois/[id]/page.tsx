...
export default function BoiDetalhePage() {
  ...
  const { boi, loading, editPeso, deletePeso, editCirc, deleteCirc, savePesoComMes, saveCircComMes } = useBoiDetail(
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="text-gray-700 bg-white shadow rounded-lg p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <div><span className="font-medium">RGN:</span> {boi.animal.rgn ?? "-"}</div>
          <div><span className="font-medium">Série/RGD:</span> {boi.animal.serieRGD ?? "-"}</div>
          <div><span className="font-medium">Sexo:</span> {boi.animal.sexo ?? "-"}</div>
          <div><span className="font-medium">Nascimento:</span> {boi.animal.nasc ?? "-"}</div>
          <div><span className="font-medium">iABCZg:</span> {boi.animal.iabcgz ?? "-"}</div>
          <div><span className="font-medium">DECA:</span> {boi.animal.deca ?? "-"}</div>
          <div><span className="font-medium">P%:</span> {boi.animal.p ?? "-"}</div>
          <div><span className="font-medium">F%:</span> {boi.animal.f ?? "-"}</div>
          <div><span className="font-medium">Pai:</span> {boi.pai?.nome ?? "-"}</div>
          <div><span className="font-medium">Mãe:</span> {`${boi.mae?.serieRGD ?? "-"} ${boi.mae?.rgn ?? ""}`.trim()}</div>
        </div>
      </div>
...
<section className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="bg-white shadow rounded-lg p-4 space-y-3">
        ...
        {(!boi.animal.pesosMedidos || boi.animal.pesosMedidos.length === 0) && (
      ...
      <div className="bg-white shadow rounded-lg p-4 space-y-3">
        ...
        {(!boi.animal.circunferenciaEscrotal || boi.animal.circunferenciaEscrotal.length === 0) && (
      ...
      <Button variant="outline" onClick={() => router.push("/bois")}>
        Voltar
      </Button>
    </div>
  );
